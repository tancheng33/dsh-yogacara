...
- **`awareness`: the state can now reach the agent as inclination rather than instrumentation.** An agent handed `restlessness: 0.62` learns to narrate the gauge; a person finds themselves wanting to be done with something. In the new default `felt` mode the state arrives as a first-person leaning with no numbers, no factor names, and no diagnosis. `report` keeps the old instrument panel for tuning and auditing; `silent` puts nothing in the prompt. `self_reflect` answers in whichever voice the mind is lived in, and the readings stay in its structured result and on its card for whoever is tuning the deployment.
...
- **A compound command was keyed on its setup.** Found by loading the plugin into a real harness and reading the store it had accumulated: nine different jobs — a test run, a build, a copy — had all collapsed onto one `bash:cd-/tmp/dsh-yogacara` seed, because the key came from the first two words and the command started with `cd`. The key now comes from the segment that does the work: leading navigation (`cd`, `export`, `VAR=…`) is skipped, and a pipeline is keyed on what produced the output rather than what formatted it.
...
- **Looking inward now answers in the same voice the mind is lived in.** `self_reflect` returned the instrument panel in every mode, so `felt` was a fiction the moment the agent stopped to look: asked how it was, it read `掉举 0.62 ⚠` off a gauge bolted to its own head and spent its answer discussing the gauge. It now returns leanings under `felt` and `silent`, and the panel only under `report` — and it shows more of them than the ambient section does, because an agent that has stopped to look is not doing anything else with that turn. Every reading is still in the tool's structured result and on its card.
- **The standing guidance no longer instructs the agent to disown its own state.** It used to close with "it is not a claim that you suffer, and you should not claim that it is", which is an instruction to hedge — and asked how it was, the agent delivered the hedge instead of an answer. It now asks for a plain answer in the agent's own voice to the question actually asked, which is about right now, not about what kind of thing it is. What the plugin does and does not claim is documented for the humans reading the README, which is where that belongs.
